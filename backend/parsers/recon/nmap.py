...
try:
    ...
    warnings.warn(
        "defusedxml not installed; nmap XML parser is vulnerable to XXE. Install with: pip install defusedxml",
        stacklevel=2,
    )
        ...
        for script_el in host_el.findall(".//script"):
            ...
            if script_id and output:
                entities.append(
                    ParsedEntity(
                        kind="nmap_script",
                        label=f"{ip}:{script_id}",
                        confidence=0.8,
                        properties={"script_id": script_id, "output": output[:2000], "host": ip},
                        source_tool="nmap",
                        phase="dns_infrastructure",
                    )
                )
