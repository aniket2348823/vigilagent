...
class CredentialVault:
    ...
    def _init_encryption(self, encryption_key: str | None) -> None:
        ...
        try:
            ...
            if not key_material:
                ...
                logger.info(
                    "[Vault] Using persisted local dev key (set VIGILAGENT_VAULT_KEY to override in production)."
                )
                return
            ...
            with open(lock_path, "w") as lock_file:
                ...
                try:
                    ...
                    try:
                        ...
                        logger.debug(
                            "[Vault] chmod failed (non-POSIX?): %s", exc
    ...
    def _encrypt(self, secret: str) -> str:
        ...
        raise RuntimeError(
            "Credential vault encryption is unavailable. Install 'cryptography' or set VIGILAGENT_VAULT_KEY."
        )
        ...
        with self._lock:
            for r in rows:
                cred = Credential(
                    ...
                    kind=r.get("kind", "token"),
                    source=r.get("source", "manual"),
                    privilege=r.get("privilege", "unknown"),
                    captured_at=r.get("captured_at", _now()),
                )
    ...
    def get_fresh_credential(
        self, target: str, *, service: str = "http", privilege: Privilege | None = None
    ) -> tuple[Credential, str] | None:
    ...
    def get_alternate_identity(
        self, target: str, exclude_principal: str | None = None
    ) -> tuple[Credential, str] | None:
    ...
    def stats(self) -> dict:
        with self._lock:
            return {
                ...
                "by_privilege": {
                    p: sum(1 for c in self._creds.values() if c.privilege == p) for p in ("unknown", "user", "admin")
                },
