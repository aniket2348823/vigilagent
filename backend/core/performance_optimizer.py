...
class PerformanceOptimizer:
    ...
    def __init__(self):
        ...
        self.operation_counts = {"http": 0, "browser": 0, "learning": 0}

        # Performance thresholds
        self.thresholds = {"slow_response_ms": 2000, "high_error_rate": 0.1, "low_throughput": 1.0}

    def record_performance(self, component: str, response_time_ms: float, success: bool):
        """Record performance metric for a component."""
        self.metrics[component].append(
            {"response_time_ms": response_time_ms, "success": success, "timestamp": time.time()}
        )
    ...
    def balance_learning_overhead(self, scan_performance_ms: float, learning_overhead_ms: float) -> dict[str, Any]:
        """
    ...
    def recommend_http_alternative(self, browser_performance_ms: float, http_performance_ms: float) -> dict[str, Any]:
        """
    ...
    def adapt_operation_mix(self, target_throughput: float) -> dict[str, float]:
        """
    ...
    def track_end_to_end_performance(self, scan_id: str, total_time_ms: float, breakdown: dict[str, float]):
        """Track end-to-end performance including all components."""
        self.metrics["end_to_end"].append(
            {"scan_id": scan_id, "total_time_ms": total_time_ms, "breakdown": breakdown, "timestamp": time.time()}
        )
        ...
        if len(self.metrics["end_to_end"]) > 10:
            ...
            if total_time_ms > avg_recent * 1.5:
                logger.warning(
                    f"[PerfOptimizer] Performance degradation detected: {total_time_ms:.0f}ms vs {avg_recent:.0f}ms avg"
                )
