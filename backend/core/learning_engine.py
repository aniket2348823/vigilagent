...
class ContinuousLearningEngine:
    ...
    def __init__(self, brain_dir: str = "brain", redis_client: Any | None = None):
    ...
    def _evict_low_confidence_patterns(self):
        ...
        sorted_patterns = sorted(self.patterns.values(), key=lambda p: (p.confidence, p.success_count))
    ...
    async def _learn_vuln_correlation(self, scan_id: str, vuln_type: str, url: str):
        ...
        try:
            ...
            raw_text = await asyncio.to_thread(episode_file.read_text, encoding="utf-8")
            ...
            for other_vuln in vuln_events:
                ...
                if other_type and other_type != vuln_type:
                    pattern_data = {"vuln_type_1": vuln_type, "vuln_type_2": other_type, "correlation": "co_occurrence"}
    ...
    def _extract_payload_features(self, payload: str) -> dict[str, Any]:
        ...
        features = {
            ...
            "has_sql_keywords": any(
                kw in payload_lower for kw in ["select", "union", "drop", "insert", " or ", "and "]
            ),
    ...
    async def get_recommendations(self, target_url: str, scan_context: dict[str, Any]) -> dict[str, Any]:
        ...
        recommendations = {"priority_vulns": [], "effective_payloads": [], "correlated_vulns": [], "confidence": 0.0}
        ...
        for vuln_rec in recommendations["priority_vulns"]:
            ...
            correlations = [
                p
                for p in self.patterns.values()
                if p.pattern_type == "vuln_correlation"
                and (p.pattern_data.get("vuln_type_1") == vuln_type or p.pattern_data.get("vuln_type_2") == vuln_type)
                and p.confidence > self.min_confidence_threshold
            ]

            for corr in correlations:
                other_type = (
                    corr.pattern_data.get("vuln_type_2")
                ...
                recommendations["correlated_vulns"].append(
                    {"primary": vuln_type, "correlated": other_type, "confidence": corr.confidence}
                )
    ...
    async def consolidate_patterns(self):
        ...
        for _pattern_type, patterns in by_type.items():
            if len(patterns) < 2:
                continue
    ...
    def _patterns_similar(self, p1: LearningPattern, p2: LearningPattern) -> bool:
        ...
        if p1.pattern_type == "endpoint_pattern":
            return p1.pattern_data.get("vuln_type") == p2.pattern_data.get("vuln_type") and p1.pattern_data.get(
                "url_pattern"
            ) == p2.pattern_data.get("url_pattern")
    ...
    def get_metrics(self) -> dict[str, Any]:
        ...
        self.metrics.high_confidence_patterns = sum(1 for p in self.patterns.values() if p.confidence > 0.7)

        if self.patterns:
            self.metrics.avg_pattern_confidence = sum(p.confidence for p in self.patterns.values()) / len(self.patterns)
    ...
    def _browser_vuln_fingerprint(self, vuln_data: dict[str, Any]) -> str:
        ...
        key_data = {
            ...
            "method": (vuln_data.get("method") or "GET").upper(),
            "engine": (
                vuln_data.get("browser_engine") or (vuln_data.get("browser_context") or {}).get("engine") or "chromium"
            ),
        }
    ...
    async def _acquire_browser_learn_lock(self, lock_key: str, ttl_seconds: int = 300) -> bool:
        ...
        try:
            ...
            result = await asyncio.wait_for(
                asyncio.to_thread(self.redis_client.set, lock_key, "1", **{"nx": True, "ex": ttl_seconds}), timeout=15
            )
            return bool(result)
        except Exception as e:
    ...
    def _extract_browser_context(self, vuln_data: dict[str, Any]) -> dict[str, Any]:
        ...
        return {
            "engine": (vuln_data.get("browser_engine") or ctx_in.get("engine") or "chromium"),
            "framework": vuln_data.get("framework") or ctx_in.get("framework"),
            "headless": bool(vuln_data.get("headless", ctx_in.get("headless", True))),
            "stealth": bool(vuln_data.get("stealth_required", ctx_in.get("stealth", False))),
            "viewport": ctx_in.get("viewport") or vuln_data.get("viewport"),
            "user_agent": ctx_in.get("user_agent") or vuln_data.get("user_agent"),
            "session_required": bool(vuln_data.get("session_required", ctx_in.get("session_required", False))),
        }
    ...
    def _extract_execution_requirements(self, vuln_data: dict[str, Any]) -> dict[str, Any]:
        ...
        return {
            "browser_engine": (vuln_data.get("browser_engine") or req_in.get("browser_engine") or "chromium"),
            "javascript_execution": bool(req_in.get("javascript_execution", vuln_data.get("requires_js", True))),
            ...
            "dom_required": bool(req_in.get("dom_required", vuln_data.get("requires_dom", True))),
            "websocket_required": bool(
                req_in.get(
                    "websocket_required",
                    vuln_data.get("vuln_type") == "websocket" or vuln_data.get("type") == "websocket",
                )
            ),
            "auth_required": bool(req_in.get("auth_required", vuln_data.get("requires_auth", False))),
        }
    ...
    async def learn_browser_workflow(
        ...
        framework = workflow.get("framework") or (workflow.get("preconditions") or {}).get("framework")
        ...
        try:
            ...
            pattern_id = self._generate_pattern_id("browser_workflow", fingerprint_data)
    ...
    def _sanitize_workflow_steps(self, raw_steps: list[Any]) -> list[dict[str, Any]]:
        ...
        for raw in raw_steps[:50]:
            ...
            if validates is not None:
                ...
                step["validates"] = validates if isinstance(validates, (list, dict)) else str(validates)[:200]
            sanitized.append(step)
        return sanitized

    def _extract_workflow_preconditions(self, workflow: dict[str, Any]) -> dict[str, Any]:
        ...
        return {
            ...
            "browser_engine": (pre_in.get("browser_engine") or workflow.get("browser_engine") or "chromium"),
    ...
    async def get_browser_recommendations(
        ...
        try:
            floor = float(target.get("confidence_floor", self.min_confidence_threshold))
        ...
        for pattern in self.patterns.values():
            ...
            if ptype != "framework_pattern":
                pattern_url = data.get("url_pattern") or (
                    self._extract_url_pattern(data.get("url") or "") if data.get("url") else ""
                )
            ...
            framework_match = framework_norm is not None and entry_framework_norm == framework_norm
        ...
        overall_conf = sum(confidence_pool) / len(confidence_pool) if confidence_pool else 0.0
    ...
    def _browser_reco_cache_get(self, key: tuple) -> dict[str, Any] | None:
        ...
        if getattr(self, "_browser_reco_cache_stamp", None) != self._browser_reco_cache_signature():
            cache.clear()
            self._browser_reco_cache_stamp = self._browser_reco_cache_signature()
            return None
        return cache.get(key)

    def _browser_reco_cache_put(self, key: tuple, value: dict[str, Any]) -> None:
        cache: dict = getattr(self, "_browser_reco_cache", None)
        if cache is None:
            self._browser_reco_cache = {}
    ...
    async def learn_framework_pattern(
        ...
        idem_key = self._framework_pattern_fingerprint(framework_norm, route_patterns)
        lock_key = f"lock:{idem_key}"
        if not await self._acquire_browser_learn_lock(lock_key, ttl_seconds=300):
            return False
    ...
    def _framework_pattern_fingerprint(self, framework: str, sorted_route_patterns: list[str]) -> str:
    ...
    def _extract_framework_route_patterns(self, routes: list[str]) -> list[str]:
...
class BrowserLearningExtension:
    ...
    async def learn_from_browser_vulnerability(self, vuln_data: dict[str, Any], scan_id: str) -> bool:
        """
    ...
    async def learn_browser_workflow(self, workflow: dict[str, Any], success: bool):
    ...
    async def get_browser_recommendations(
        self, target_url: str, context: dict[str, Any] | None = None
    ) -> dict[str, Any]:
        """Get browser-specific recommendations based on learned patterns"""
        context = context or {}

        recommendations = {"workflows": [], "payloads": [], "framework_specific": [], "confidence": 0.0}
    ...
    async def learn_framework_pattern(self, framework: str | None, routes: list[str]):
        ...
        if self.redis:
            try:
                ...
                existing_routes = set(json.loads(existing_data)) if existing_data else set()
            except Exception as redis_exc:
...
class CrossSystemLearningExtension:
    ...
    async def recommend_cross_method_verification(
        self, vuln_data: dict[str, Any], discovery_method: str
    ) -> dict[str, Any]:
        """
        ...
        """
        recommendations = {"should_verify": False, "verification_method": None, "confidence": 0.0, "reasoning": []}
    ...
    async def identify_cross_context_patterns(
        self, http_patterns: list[LearningPattern], browser_patterns: list[LearningPattern]
    ) -> list[dict[str, Any]]:
        """
    ...
    async def extract_http_payload_from_browser_workflow(self, workflow: dict[str, Any]) -> dict[str, Any] | None:
        """
    ...
    async def track_http_browser_correlation(
        self, http_result: dict[str, Any], browser_result: dict[str, Any], target_url: str
    ):
        """
        ...
        total_endpoints = correlation_data["http_endpoints_found"] + correlation_data["browser_endpoints_found"]

        if total_endpoints > 0:
            correlation_data["overlap_rate"] = correlation_data["overlap_endpoints"] / total_endpoints
            correlation_data["browser_unique_rate"] = correlation_data["unique_browser_endpoints"] / total_endpoints
        else:
        ...
        if pattern_id in self.engine.patterns:
            ...
            pattern.pattern_data["overlap_rate"] = (
                old_data.get("overlap_rate", 0) * pattern.success_count + correlation_data["overlap_rate"]
            ) / (pattern.success_count + 1)
            pattern.pattern_data["browser_unique_rate"] = (
                old_data.get("browser_unique_rate", 0) * pattern.success_count + correlation_data["browser_unique_rate"]
            ) / (pattern.success_count + 1)
        else:
            pattern = LearningPattern(
                pattern_id=pattern_id,
    ...
    def get_correlation_stats(self) -> dict[str, Any]:
        ...
        correlation_patterns = [
            p for p in self.engine.patterns.values() if p.pattern_type == "http_browser_correlation"
        ]

        if not correlation_patterns:
            return {"total_correlations": 0, "avg_overlap_rate": 0.0, "avg_browser_unique_rate": 0.0}

        total = len(correlation_patterns)
        avg_overlap = sum(p.pattern_data.get("overlap_rate", 0) for p in correlation_patterns) / total
        avg_unique = sum(p.pattern_data.get("browser_unique_rate", 0) for p in correlation_patterns) / total
...
class AuthenticationPatternLearner:
    ...
    def extract_auth_pattern(self, auth_data: dict[str, Any], success: bool) -> dict[str, Any] | None:
        """
    ...
    def store_auth_pattern_as_skill(self, pattern: dict[str, Any], skill_library: Any) -> Any:
        ...
        skill = BrowserSkill(
            ...
            execution_context="browser_required",
            browser_requirements={"session": True, "stealth": False},
    ...
    def reuse_auth_pattern(self, target_domain: str, auth_method: str = "form") -> dict[str, Any] | None:
        """
    ...
    def store_session_state(self, target_domain: str, session_data: dict[str, Any]):
    ...
    def get_session_state(self, target_domain: str) -> dict[str, Any] | None:
    ...
    async def re_authenticate(self, target_domain: str, browser_orchestrator: Any, credentials: dict[str, str]) -> bool:
        """
        ...
        try:
            ...
            for _step in pattern.get("steps", []):
                # Execute step
                pass
    ...
    def track_auth_method_effectiveness(self, target_type: str, auth_method: str, success: bool):
        ...
        stats = json.loads(stats_json) if stats_json else {"success_count": 0, "failure_count": 0}
    ...
    def get_auth_stats(self) -> dict[str, Any]:
        ...
        return {
            ...
            "active_sessions": sum(1 for s in self.session_store.values() if time.time() < s["expires_at"]),
            "timestamp": time.time(),
        }


# Global authentication pattern learner
auth_pattern_learner: AuthenticationPatternLearner | None = None
