...
class CapabilityAssessor:
    ...
    async def get_proficiency(self, skill: str) -> float:
        ...
        return self._proficiency_scores.get(skill, self.config.initial_proficiency)

    async def update_proficiency(self, skill: str, outcome: bool, context: dict[str, Any]) -> None:
        """
        ...
        """
        with trace_span(
            "capability_assessor.update_proficiency", {"agent_id": self.agent_id, "skill": skill, "outcome": outcome}
        ) as span:
            await self._ensure_loaded()
    ...
    async def can_perform(self, skill: str, min_proficiency: float | None = None) -> bool:
        """
    ...
    async def check_prerequisites(self, action: Any, context: dict[str, Any]) -> PrerequisiteCheck:
        """
        ...
        if not prerequisites:
            return PrerequisiteCheck(satisfied=True, missing_prerequisites=[], rationale="No prerequisites required")
    ...
    async def suggest_delegation(self, skill: str) -> str | None:
        """
        ...
        try:
            ...
            result = await db_manager._run_sync(
                lambda: (
                    db_manager.supabase.table("agent_proficiency")
    ...
    async def _load_from_db(self):
        ...
        try:
            ...
            logger.info(
                f"[CapabilityAssessor] Loaded {len(self._proficiency_scores)} proficiency scores for {self.agent_id}"
            )
