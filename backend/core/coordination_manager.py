...
class CoordinationManager:
    ...
    def __init__(self, agent_id: str, agent: "BaseAgent", config: SelfAwarenessConfig):
    ...
    async def broadcast_status(self, status: dict[str, Any]):
        ...
        try:
            event = HiveEvent(
                ...
                payload={"message_type": HiveMessageType.STATUS_UPDATE.value, "status": status},
            )
    ...
    async def delegate_task(self, task: Any, target_agent: str | None = None) -> DelegationResult:
        """Delegate task to another agent"""
        if not target_agent:
            target_agent = await self.select_best_agent(task)

        if not target_agent:
            return DelegationResult(delegated=False, rationale="No suitable agent found")
        ...
        try:
            ...
            return DelegationResult(delegated=True, target_agent=target_agent, rationale=f"Delegated to {target_agent}")
        except Exception as e:
            logger.error(f"[CoordinationManager] Delegation failed: {e}")
            return DelegationResult(delegated=False, rationale=str(e))

    async def select_best_agent(self, task: Any) -> str | None:
    ...
    async def request_assistance(self, problem: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            ...
            return {"requested": True, "problem": problem, "timestamp": event.timestamp.isoformat()}
